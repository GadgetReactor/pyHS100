...
CURRENT_CONSUMPTION_SCHEMA = Schema(Any({
    ...
    'voltage_mv': Any(All(float, Range(min=0, max=300000)), int, None), # TODO can this be int?
    'power_mw': Any(Coerce(float, Range(min=0)), None),
    'total_wh': Any(Coerce(float, Range(min=0)), None),
    'current_ma': Any(All(float, Range(min=0)), int, None), # TODO can this be int?
}, None))
...
class FakeTransportProtocol(TPLinkSmartHomeProtocol):
    def __init__(self, info, invalid=False):
        ...
        for module in ["emeter", "smartlife.iot.common.emeter"]:
            ...
            if "get_realtime" in info[module]:
                ...
                if "err_code" not in get_realtime_res or not get_realtime_res["err_code"]:
    ...
    def transition_light_state(self, x):
        ...
        light_state = self.proto["smartlife.iot.smartbulb.lightingservice"]["get_light_state"]
        ...
        if "on_off" in x:
            if x["on_off"] and not light_state["on_off"]: # turning on
                ...
                new_state["on_off"] = 1
                self.proto["smartlife.iot.smartbulb.lightingservice"]["get_light_state"] = new_state
    ...
    baseproto = {
        ...
        "smartlife.iot.smartbulb.lightingservice": {"get_light_state": light_state,
